...
def setup_app_state(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    db_path = tmp_path / "atlas-haven-kolibri.db"
...
def test_kolibri_smoke_installs_with_launch_url() -> None:
    CatalogSyncService().sync_from_disk()
    manager = SystemServiceManager()

    manager.install_service("nomad_kolibri")

    service = next(
        item for item in manager.list_services(installed_only=True) if item.service_name == "nomad_kolibri"
    )
    assert service.status == "running"
    assert service.launch_url == "http://127.0.0.1:8085"
