...
def setup_app_state(monkeypatch: pytest.MonkeyPatch, tmp_path: Path):
    db_path = tmp_path / "atlas-haven-cyberchef.db"
...
def test_cyberchef_smoke_installs_with_launch_url() -> None:
    CatalogSyncService().sync_from_disk()
    manager = SystemServiceManager()

    manager.install_service("nomad_cyberchef")

    service = next(
        item for item in manager.list_services(installed_only=True) if item.service_name == "nomad_cyberchef"
    )
    assert service.status == "running"
    assert service.launch_url == "http://127.0.0.1:8001"
